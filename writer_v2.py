...
CURIOSITY GAP (hard requirement): one of the first 3 beats must literally end in a question mark "?" -- a genuine question the story then actually answers by the payoff, not a rhetorical throwaway. A script with no "?" in the first 3 beats is invalid. The HOOK is never the question: a question-mark hook is rejected outright by the validator (see the HOOK rule above), so put the question in beat 1, 2, or 3.
...
def render_length_contract(contract):
    ...
    c = contract
    return (
        "\n\nLENGTH CONTRACT (mechanically enforced -- a script outside these bounds is thrown "
        "out outright, not politely trimmed for you):\n"
