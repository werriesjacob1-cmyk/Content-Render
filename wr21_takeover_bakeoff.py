...
def _hp_for_round(r,fact): return HP.payoff_proof_report(hook=r.get("hook") or "",payoff=r.get("payoff") or "",central_question=(fact or {}).get("whatif") or "")
...
def _render_manifest_story(m):
    scenes=list(m.get('scenes') or []); print('  FINAL RENDER-MANIFEST STORY:')
    ...
    hp=HP.manifest_hook_payoff_report(m); vf=VF.visual_feasibility_report(scenes)
    ...
    print(f"    VISUAL FEASIBILITY: {json.dumps(vf,ensure_ascii=False,sort_keys=True)}")
...
def run_topic(tid,fact):
    ...
    vf=_render_manifest_story(manifest) if manifest else {"warning_kinds":[],"problem_scene_count":0}
    if not manifest: print(f"  ABORTED: validate_err={debug.get('validate_err')!r}")
