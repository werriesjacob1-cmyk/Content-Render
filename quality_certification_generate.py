...
def _quality_lane_profile(
    ...
    joint_score, treatment, treatment_evidence = QSS.score_topic(
        fact,
    ...
    # Existing visual tellability still dominates, but a topic-treatment pair
    # that is structurally hard to write can no longer win solely on B-roll.
    quality_score = (
        scout_score * 0.50
        + joint_score * 0.30
        + min(nasa_hits + pubchem_hits, 2) * 0.75
        + (0.35 if motion else 0.0)
        + min(len(queries), 4) * 0.12
        + min(len(key_terms), 4) * 0.08
    )
