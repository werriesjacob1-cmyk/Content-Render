...
| 2 | Serialized loops / follow-hook | **BUILT (2026-08-03)** | `generate.py` `_pick_series`/`_find_callback`: SERIES mode now activates AUTOMATICALLY off the chosen fact's domain (no more manual env var) — 6 curated themes (`SERIES_THEMES`), continuation always wins over starting fresh, capped at `SERIES_MAX_PARTS`=4 to avoid staleness, persisted per-video in `memory_science.json`'s `series` field so the next render can detect it. Videos NOT in a series get a lighter cross-video callback instead when a same-domain prior video exists ("remember the ocean-floor one? this is why..."). Endings still "leave the page open" as before. NOT yet render-verified — watch for `[series] ... — part N` or `[series] callback to ...` in the generate log and confirm the on-screen "Part N" label + the tease line actually render well. |
