...
A pre-search visual bible is made load-bearing in Visual Director/science-motion
routing, and actual per-scene asset lineage records what finally rendered.
"""
...
def _augment_provenance(motion_plans, motion_state) -> None:
    ...
    for sid, plan in motion_plans.items():
        ...
        rows.append(row)
    payload["schema"] = "quality-render-bridge-v6-visual-intent-lineage"
...
def _write_scene_timeline(manifest: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for idx, scene in enumerate(scenes, 1):
        ...
        rows.append({
            "scene_id": B._scene_key(scene, idx), "scene_index": idx,
...
def main() -> None:
    ...
    directed_manifest = QVB.apply_visual_bible(manifest, visual_bible)
    print(f"[quality-science] visual bible sealed+applied for {len(visual_bible.get('scenes') or [])} scenes BEFORE asset search")

    allowed_claim_ids = WSB.manifest_claim_ids(manifest)
    planned = QSM.plan_manifest(directed_manifest, allowed_claim_ids)

    allow_free_network = os.getenv("QUALITY_RENDER_FREE_NETWORK", "") == B.FREE_NETWORK_ACK
    injections, decisions = B.resolve_manifest_assets(directed_manifest, allow_free_network=allow_free_network)
    ...
    print(f"[quality-science] deterministic motion planned={len(planned)} active={len(active_motion)}; authentic injections={len(injections)}")

    # Install against the canonical manifest objects; injections are keyed by
    # scene ID, while all routing decisions above were made from the visual-
    # directed copy. Spoken narration/claim identity therefore never mutates.
    bridge_restore = B.install_bridge(injections, manifest)
    bridged_build_scene = legacy.build_scene
    motion_state = {sid: {"rendered": False} for sid in active_motion}
    lineage_state: dict[str, dict[str, Any]] = {}

    def _record_lineage(scene, idx, out_path, *, renderer_kind: str, before_ids: list[str], science_plan=None, notes: str = ""):
        ...
        lineage_state[sid] = {"entry": QAL.AssetLineageEntry(
            ...
            renderer_kind=actual_kind, output_file=str(out_path or ""), duration_s=round(duration, 3),
    ...
    def science_build_scene(scene, idx, seg_mp3, seg_dur):
        ...
        lineage_state[sid] = {"before_stat": int(getattr(legacy, "STAT_CARD_SCENES", 0)),
        ...
        if plan is not None:
            try:
                ...
                _record_lineage(scene, idx, out, renderer_kind="deterministic_science_motion", before_ids=before_ids, science_plan=plan)
                print(f"  [science-motion] deterministic {plan.spec.kind.value} scene {sid} from claims {list(plan.source_claim_ids)}")
                return out
        ...
        _record_lineage(scene, idx, out, renderer_kind=kind, before_ids=before_ids, notes="quality bridge/legacy final selector")
        return out
    ...
    try:
        ...
        try:
            ...
            print(f"[quality-science] measured {timeline['scene_count']} scene boundaries across {timeline['measured_body_duration_s']:.3f}s")
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            print(f"[quality-science] asset lineage attributable={lineage['attributable_scene_count']}/{lineage['scene_count']}")
        except Exception as exc:  # noqa: BLE001
